...
問題本文が完成し、対応する解答ファイルが明示的に「準備中」の回は、問題本文だけをDRAFT登録します。解答完成後に再取込すると同じ問題コードへ解答・解説が追加されます。

## 模試作成

1. `/admin/mocks/new` で初期設定プリセットまたはテンプレートを選び、試験時間・大問数・紙面を設定します。既存模試の詳細にある「複製して編集」から構成を引き継ぐこともできます。
2. 作成された第1問、第2問…の「この大問の候補条件」で分野、サブ分野、難易度、未使用のみを設定します。
3. 「候補を選ぶ」からキーワード・大学・想定時間・検証状態を追加指定し、問題全文を比較して配置します。模試の科目と想定大学は初期条件として自動適用されます。
4. 「差し替える」「外す」「↑」「↓」で構成を調整します。大問の追加、末尾の空欄削除、空欄だけの条件付き仮配置も利用できます。
5. 上部の合計時間、試験時間との差、平均難易度、分野構成、他模試での使用数を確認します。同じ問題を一つの模試内へ重複配置することはできません。
6. 全大問を配置すると状態を「完成」にできます。問題冊子・解答冊子・両方のプレビューを確認して印刷または書き出します。

...
export default async function ProblemBankPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <>
    <AdminPageHeader eyebrow="PROBLEM BANK" title="問題データベース" description={`${total}問。条件を組み合わせて候補をすばやく絞り込めます。`} action={<Link className="btn-primary" href="/admin/problems/new">＋ 問題を登録</Link>} />
    <div className="workbench space-y-5">
      ...
      <div className="card overflow-hidden">
        {rows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="bg-surface text-xs text-muted"><tr><th className="px-5 py-3">問題</th><th className="px-4 py-3">分類</th><th className="px-4 py-3">難易度</th><th className="px-4 py-3">時間</th><th className="px-4 py-3">検証</th><th className="px-4 py-3">使用</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-line">{rows.map(({ problem, subjectName, usageCount }) => <tr key={problem.id} className="hover:bg-slate-50"><td className="px-5 py-4"><strong className="block text-navy">{problem.code}</strong><span className="mt-1 block max-w-xl text-xs leading-5 text-muted">{excerpt(problem.statement)}</span></td><td className="px-4 py-4"><strong className="block text-xs text-navy">{subjectName || "未分類"}</strong><span className="mt-1 block text-xs text-muted">{problem.field}{problem.subfield ? ` / ${problem.subfield}` : ""}</span><span className="mt-1 block text-xs text-muted">{problem.targetUniversity || "想定大学なし"}</span></td><td className="px-4 py-4 font-black text-navy">{problem.difficulty} / 5</td><td className="px-4 py-4 text-xs text-muted">{problem.estimatedMinutes}分</td><td className="px-4 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${problem.verificationStatus === "VERIFIED" ? "bg-teal-50 text-teal" : "bg-slate-100 text-muted"}`}>{verificationLabels[problem.verificationStatus]}</span></td><td className="px-4 py-4 text-xs font-bold text-muted">{Number(usageCount)}回</td><td className="px-5 py-4 text-right"><Link className="font-bold text-brand-blue hover:underline" href={`/admin/problems/${problem.id}`}>詳細 →</Link></td></tr>)}</tbody></table></div> : <div className="grid min-h-64 place-items-center p-8 text-center"><div><h2 className="font-black text-navy">条件に合う問題がありません</h2><p className="mt-2 text-sm text-muted">条件を変更するか、新しい問題を登録してください。</p></div></div>}
      </div>

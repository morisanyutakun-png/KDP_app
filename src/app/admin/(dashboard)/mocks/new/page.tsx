  ...
  return <>
    ...
    <div className="workbench grid max-w-6xl items-start gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-4">
        <nav aria-label="初期設定" className="flex flex-wrap items-center gap-x-1 border-b border-line pb-3"><span className="mr-3 text-xs font-bold text-muted">初期設定</span>{Object.entries(presets).map(([key, preset]) => <Link className={`border-b-2 px-3 py-2 text-sm font-bold ${selectedPreset === preset ? "border-navy text-navy" : "border-transparent text-muted hover:text-navy"}`} href={`/admin/mocks/new?preset=${key}`} key={key}>{preset.label}<span className="ml-2 text-xs font-normal">{preset.durationMinutes}分 / {preset.paperSize}</span></Link>)}</nav>

        <form action={createMockExamAction} className="card space-y-6 p-5 sm:p-7">
          <section><h2 className="font-bold text-navy">基本情報</h2><div className="mt-4 space-y-4"><label><span className="label">模試名 *</span><input className="input" name="title" required placeholder="名古屋大学 数学予想模試 第1回" /></label><div className="grid gap-4 sm:grid-cols-2"><label><span className="label">科目</span><select className="input" name="subjectId" defaultValue=""><option value="">未設定</option>{subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label><label><span className="label">想定大学・試験</span><input className="input" list="new-mock-university-options" name="targetUniversity" placeholder="名古屋大学 / 大学入学共通テスト" /></label><label><span className="label">試験時間（分）</span><input className="input" name="durationMinutes" type="number" min="1" max="600" defaultValue={selectedPreset.durationMinutes} /></label><label><span className="label">大問数</span><input className="input" name="questionCount" type="number" min="1" max="20" defaultValue={selectedPreset.questionCount} /></label></div></div></section>

          <section className="border-t border-line pt-5"><label><span className="label">保存済みテンプレート</span><select className="input" name="templateId" defaultValue=""><option value="">テンプレートを使用しない</option>{templates.map(({ template }) => <option key={template.id} value={template.id}>{template.name}（{template.questionCount}問 / {template.durationMinutes}分 / {template.paperSettings.paperSize}）</option>)}</select><span className="mt-1.5 block text-xs text-muted">選択時は科目・対象・時間・問題数・紙面設定をテンプレートから読み込みます。</span></label></section>

          <details className="border-t border-line pt-5"><summary className="cursor-pointer text-sm font-bold text-navy">紙面設定を変更 <span className="ml-2 font-normal text-muted">{selectedPreset.paperSize} · 11pt · 余白16mm</span></summary><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><label><span className="label">用紙</span><select className="input" name="paperSize" defaultValue={selectedPreset.paperSize}><option value="B5">B5</option><option value="A4">A4</option></select></label><label><span className="label">文字サイズ</span><input className="input" name="fontSize" type="number" min="9" max="14" defaultValue="11" /></label><label><span className="label">余白（mm）</span><input className="input" name="marginMm" type="number" min="8" max="35" defaultValue="16" /></label><label><span className="label">段組</span><select className="input" name="columns" defaultValue="1"><option value="1">1段</option><option value="2">2段</option></select></label></div><div className="mt-4 flex flex-wrap gap-5 text-sm"><label className="flex items-center gap-2"><input type="checkbox" name="showPageNumbers" defaultChecked />ページ番号</label><label className="flex items-center gap-2"><input type="checkbox" name="pageBreakPerProblem" />問題ごとに改ページ</label></div></details>

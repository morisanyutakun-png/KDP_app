...
export function MaterialForm({ material, error }: { material?: AdminMaterial; error?: string }) {
  return <form action={saveMaterialAction} className="space-y-6">
    ...
    {error && <div role="alert" className="rounded-md border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</div>}
    <section className="card p-5 sm:p-6"><h2 className="text-lg font-bold text-navy">基本情報</h2><div className="mt-5 grid gap-5 sm:grid-cols-2">
    ...
    <section className="card p-5 sm:p-6"><h2 className="text-lg font-bold text-navy">画像・ファイル</h2><p className="mt-1 text-xs text-muted">Vercel Blobへブラウザから直接アップロードします。</p><div className="mt-5 grid gap-5 sm:grid-cols-2"><FileUploadField name="coverUrl" kind="cover" label="表紙画像（10MBまで）" accept="image/jpeg,image/png,image/webp,image/avif" initialUrl={material?.coverUrl} /><FileUploadField name="samplePdfUrl" kind="sample" label="サンプルPDF（50MBまで）" accept="application/pdf" initialUrl={material?.samplePdfUrl} /></div></section>

    <section className="card p-5 sm:p-6"><h2 className="text-lg font-bold text-navy">制作・公開状態</h2><div className="mt-5 grid gap-5 sm:grid-cols-2"><div><label className="label" htmlFor="productionStatus">制作状況</label><select className="input" id="productionStatus" name="productionStatus" defaultValue={material?.productionStatus || "PLANNING"}>{productionStatusValues.map((value) => <option key={value} value={value}>{productionStatusLabels[value]}</option>)}</select></div><div><label className="label" htmlFor="kdpStatus">教材全体のKDP状態</label><select className="input" id="kdpStatus" name="kdpStatus" defaultValue={material?.kdpStatus || "DRAFT"}>{kdpStatusValues.map((value) => <option key={value} value={value}>{kdpStatusLabels[value]}</option>)}</select></div><label className="flex items-center gap-3 rounded-md border border-line p-4 text-sm font-bold text-navy"><input className="size-4 accent-teal" type="checkbox" name="isPublished" defaultChecked={material?.isPublished} />公開カタログに表示</label><label className="flex items-center gap-3 rounded-md border border-line p-4 text-sm font-bold text-navy"><input className="size-4 accent-teal" type="checkbox" name="isFeatured" defaultChecked={material?.isFeatured} />注目教材として表示</label></div></section>

    <section className="card p-5 sm:p-6"><h2 className="text-lg font-bold text-navy">販売形式</h2><p className="mt-1 text-xs text-muted">ASINは売上CSVとの自動紐付けに使用します。Amazon URLが空の場合、公開ページに購入ボタンは表示されません。</p><div className="mt-5 space-y-4">{formatValues.map((format) => { const edition = material?.editions.find((item) => item.format === format); return <fieldset key={format} className="rounded-lg border border-line p-4"><legend className="px-2 font-bold text-navy">{formatLabels[format]}</legend><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><div><label className="label" htmlFor={`${format}-asin`}>ASIN</label><input className="input" id={`${format}-asin`} name={`edition.${format}.asin`} defaultValue={edition?.asin || ""} /></div><div><label className="label" htmlFor={`${format}-isbn`}>ISBN</label><input className="input" id={`${format}-isbn`} name={`edition.${format}.isbn`} defaultValue={edition?.isbn || ""} /></div><div className="md:col-span-2"><label className="label" htmlFor={`${format}-url`}>Amazon URL</label><input className="input" id={`${format}-url`} name={`edition.${format}.amazonUrl`} type="url" placeholder="https://..." defaultValue={edition?.amazonUrl || ""} /></div><div><label className="label" htmlFor={`${format}-price`}>Amazon価格</label><div className="flex"><span className="grid place-items-center rounded-l-xl border border-r-0 border-line bg-surface px-3 text-sm font-bold text-muted">¥</span><input className="input rounded-l-none" id={`${format}-price`} name={`edition.${format}.priceAmount`} type="number" min="0" step="1" defaultValue={edition?.priceAmount ?? ""} /></div><input type="hidden" name={`edition.${format}.priceCurrency`} value="JPY" />{edition?.priceUpdatedAt && <p className="mt-1 text-xs text-muted">価格確認: {new Intl.DateTimeFormat("ja-JP").format(new Date(edition.priceUpdatedAt))}</p>}</div><div><label className="label" htmlFor={`${format}-kdp`}>KDP状態</label><select className="input" id={`${format}-kdp`} name={`edition.${format}.kdpStatus`} defaultValue={edition?.kdpStatus || "DRAFT"}>{kdpStatusValues.map((value) => <option key={value} value={value}>{kdpStatusLabels[value]}</option>)}</select></div><label className="flex items-center gap-2 self-end pb-3 text-sm font-bold text-navy"><input className="size-4 accent-teal" type="checkbox" name={`edition.${format}.isActive`} defaultChecked={edition?.isActive ?? true} />有効な販売形式</label></div></fieldset>; })}</div></section>

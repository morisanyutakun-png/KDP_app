...
type CardMaterial = {
  ...
  editions?: Array<{ id: string; format: keyof typeof formatLabels; priceAmount?: number | null; priceCurrency?: string }>;
};

export function MaterialCard({ material }: { material: CardMaterial }) {
  const pricedEdition = material.editions?.find((edition) => edition.priceAmount !== null && edition.priceAmount !== undefined);
  return (
    <article className="group card flex h-full flex-col overflow-hidden transition duration-300 hover:-translate-y-1 hover:shadow-[0_14px_40px_rgba(11,29,74,0.12)]">
      <Link href={`/materials/${material.slug}`} className="relative block aspect-[3/4] overflow-hidden bg-gradient-to-br from-slate-100 to-blue-50">
        {material.coverUrl ? (
